...
echo "[Cloud Control] projectname $projectname taken from directory name\n";
...
foreach (glob("skel/templates/{,.}*-dist", GLOB_BRACE) as $distfile) {
    ...
    echo "[Cloud Control] creating clean file ($target) from dist ($distfile)...\n";
    copy($distfile, $target);

    // Then we apply our replaces for within those templates.
    echo "[Cloud Control] applying variables to $target...\n";
    applyValues($target, $replaces);
}
echo "[Cloud Control] removing dist files\n";
